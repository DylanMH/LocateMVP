  ...
  const loadTech = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        logger.error("[Manager TechDetail] Failed to load tech:", e);
      ...
      try {
        ...
        logger.log("[Manager TechDetail] Loaded tickets:", ticketsData.tickets?.length || 0);
        setTechTickets(ticketsData.tickets || []);
      } catch (e) {
        logger.error("[Manager TechDetail] Failed to load tickets:", e instanceof Error ? e.message : e);
        setTechTickets([]);
      }
    },
    [token, id],
          ...
          <Pressable
            onPress={() => {
              if (router.canGoBack()) {
                ...
                router.replace("/(manager)/(tabs)/teams" as any);
              }
            }}
            hitSlop={10}
          >
